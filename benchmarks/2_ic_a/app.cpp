...
void run_cpu(datatype *input, datatype *output, int w, int h, int f){
...
double run_gpu(datatype *h_i_vector, datatype *h_o_vector, datatype *h_o_vector_ref,\
			int w, int h, int f, int kernel_id, bool verify)throw(std::string){	
	cl_mem d_i_vector, d_o_vector;
	int number_elements_out = w * h;
	int number_elements_in = (w+f) * (h+f);
	try{
		...
		_clSetArgs(kernel_id, args_idx++, &f, sizeof(int));
		...
		return (double)(((double)w*(double)h*(double)(f*f+1))*sizeof(datatype))/(double)deltaT;		
	}
...
int main(int argc, char * argv[]){
	...
	try{
		...
		if(verify){
				...
				run_cpu<datatype>(h_i_vector, h_o_vector_ref, w, h, f);
		}
		for(int k=0; k<total_kernels; k++){
			avg_time = 0.0;
			std_time = 0.0;
			for(int r=0; r<number_trials; r++){
				trials[r] = run_gpu<datatype>(h_i_vector, h_o_vector, h_o_vector_ref, w, h, f, k, verify);
				avg_time += trials[r];
			}			
